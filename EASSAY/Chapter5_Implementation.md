# Chương 5: Triển Khai Thuật Toán Hồi Quy Đa Thức (Polynomial Regression)
...
## 5.1 Chuẩn Bị và Tiền Xử Lý Dữ Liệu
...
### 5.1.1 Bước 1 — Khám Phá Dữ Liệu (EDA)

Bộ dữ liệu Auto MPG bao gồm **398 mẫu** và **9 đặc trưng** (bao gồm biến mục tiêu `mpg`), được thu thập từ các thử nghiệm xe hơi từ thập niên 1970–1982. Mục tiêu của bước Exploratory Data Analysis (EDA) là xây dựng một bức tranh toàn diện về chất lượng dữ liệu, phân phối các biến và mối quan hệ tiềm ẩn giữa chúng — từ đó đưa ra định hướng kỹ thuật cho các bước tiếp theo.

**Phân phối của các biến số:**

![Phân phối các biến số](../Figures/EDA/distributions.png)

*Hình 5.1: Biểu đồ phân phối các biến trong tập dữ liệu Auto MPG.*
...
*Hình 5.2: Boxplot phát hiện ngoại lệ trong các biến số.*
...
*Hình 5.3: Ma trận tương quan giữa các biến.*
...
*Hình 5.4: Mối quan hệ giữa các đặc trưng chính và biến mục tiêu `mpg`.*
...
### 5.1.2 Bước 2 — Làm Sạch Dữ Liệu (Data Cleaning)
...
# Xóa cột định danh car_name — không có giá trị dự đoán
...
### 5.1.3 Bước 3 — Kỹ Thuật Đặc Trưng (Feature Engineering)
...
*Hình 5.5: Tương quan của đặc trưng `weight_per_hp` mới tạo với biến mục tiêu `mpg`.*
...
### 5.1.4 Bước 4 — Phân Chia Dữ Liệu (Data Split)
...
*Hình 5.6: Density plot so sánh phân phối biến mục tiêu `mpg` trên 3 tập Train, Validation và Test.*
...
## 5.2 Xây Dựng và Lựa Chọn Mô Hình
...
### 5.2.1 Bước 5 — Mô Hình Cơ Sở (Baseline Model)
...
*Hình 5.7: Biểu đồ Actual vs Predicted (trái) và Residuals (phải) của mô hình Baseline.*
...
### 5.2.2 Bước 6 — Lựa Chọn Mô Hình (Model Selection)
...
*Hình 5.8: Biểu đồ so sánh Train RMSE và Validation RMSE của 5 ứng viên.*
...
### 5.2.3 Bước 7 — Tinh Chỉnh Siêu Tham Số (Hyperparameter Tuning)
...
*Hình 5.9: Validation Curve — biểu đồ RMSE theo giá trị alpha trên thang logarit.*
...
### 5.2.4 Bước 8 — Huấn Luyện Mô Hình Cuối Cùng (Final Model Training)
...
### 5.3.1 Bước 9 — Đánh Giá Trên Tập Test (Evaluation Metrics)
...
*Hình 5.10: Actual vs Predicted (trái) và phân phối sai số (phải) trên tập Test.*
...
### 5.3.2 Bước 10 — Kiểm Định Chéo K-Fold (Cross Validation)
...
*Hình 5.11: Boxplot R² và RMSE từ 10 lần Cross Validation.*
...
### 5.3.3 Bước 11 — Quản Lý Thí Nghiệm (Experiment Management)
...
### 5.3.4 Bước 12 — Kiểm Định Thống Kê (Statistical Validation)
...
*Hình 5.12: Pointplot so sánh R² của Linear và Polynomial trên từng Fold.*
...
### 5.3.5 Bước 13 — Phân Tích Lỗi (Error Analysis)
...
*Hình 5.13: Phân bố sai số theo MPG thực tế (trái) và theo trọng lượng & xuất xứ (phải).*
...
## 5.4 Giải Thích Mô Hình (Model Interpretability — Bước 14)
...
*Hình 5.14: Biểu đồ Feature Importance — hệ số Ridge của 66 đặc trưng đa thức, sắp xếp theo độ lớn.*
...
Từ 14 bước triển khai, năm bài học cốt lõi được rút ra. **Bài học 1:** Pipeline là công cụ bắt buộc — không phải tùy chọn — để ngăn chặn Data Leakage; Scaler và PolynomialFeatures phải nằm bên trong Pipeline, không được fit trên dữ liệu test. **Bài học 2:** Polynomial bậc 3 là cái bẫy kinh điển của Overfitting — Val RMSE = 68.1 là lời cảnh báo rõ ràng rằng việc tăng độ phức tạp không kiểm soát sẽ phá hủy khả năng tổng quát hóa. **Bài học 3:** Ridge Regularization không chỉ là kỹ thuật bổ trợ mà là điều kiện tiên quyết khi làm việc với không gian đặc trưng đa thức bậc cao — nơi đa cộng tuyến giữa các đặc trưng là không thể tránh khỏi. **Bài học 4:** Feature Engineering từ domain knowledge (`weight_per_hp`) tạo ra đặc trưng quan trọng nhất của mô hình — vượt qua tất cả đặc trưng gốc, khẳng định rằng hiểu biết về bài toán quan trọng không kém thuật toán. **Bài học 5:** Giới hạn của mô hình (R² ≈ 87%) là trần vật lý do thiếu dữ liệu về công nghệ xe, không phải điểm yếu của thuật toán — việc nhận biết giới hạn này là biểu hiện của tư duy khoa học trưởng thành.
